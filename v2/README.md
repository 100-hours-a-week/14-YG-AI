...
**뭉치면 산다(뭉산) AI 어시스턴트**는 LangGraph 기반 멀티 에이전트 시스템을 활용하여 자연어로 공동구매 플랫폼의 기능을 이용할 수 있는 지능형 대화형 AI입니다.
...
graph TD
    A[사용자 쿼리] --> B[LLM 분류기]
    B --> C{검색 타입 결정}
    C -->|keyword| D[MySQL 직접 검색]
    C -->|vector| E[PostgreSQL 벡터 검색]
    E --> F[유사도 상위 ID 추출]
    F --> G[MySQL ID 기반 조회]
    D --> H[최종 결과 반환]
    G --> H
    
    I[MySQL 원본 데이터] --> J[1시간 배치]
    J --> K[PostgreSQL 임베딩 동기화]
```
...
**개발팀**: Moongsan-AI-Team  
**버전**: 5.0.0-hitl  
**업데이트**: 2025-07-20

---

*뭉치면 산다! V2 챗봇으로 더 쉽고 재미있는 공구 경험을 만들어보세요! 🚀*
